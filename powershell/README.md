# Sphereon.SDK.Pdf - the PowerShell module for the PDF
...
- API version: 1.1
- SDK version: 1.1.1
- Build date: 2018-02-12T19:40:52.859+01:00
...
## Dependencies
- C# API client generated by Swagger Codegen AND should be located in ..\\csharp-net45 as stated in Build.ps1
...
Then import module from the .\src\Sphereon.SDK.Pdf folder:
```powershell
...
```powershell
$conversionSettings = New-PDFConversionSettings -engine "PREMIUM" -version "_15"
$jobResponse = Submit-PDFCreateJob -settings $conversionSettings
$job = $jobResponse.Job
Write-Host $job | ConvertTo-JSON

# Add input stream...
# See test directory for integration test example
```
